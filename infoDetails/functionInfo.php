...
function getChquePaymentInfo($designParam) {
    ...
    try {
        ...
        $query = "SELECT pyh.PHID,pyh.PMID,pyh.ShopID,pyh.PayDate,pm.PaymentMethod,sp.shopName,sp.contact_no1,obi.name As OwnerBankName,
                obi.branch AS OwneBnkBranch,obi.account_no As OwnerAccountNo,cdi.CheqDId,cdi.ChqNo,cdi.ChequeAmount,
                cdi.ChequeDate,cdi.ChequeIssuedBnkId,cdi.DeptBnkId,cdi.ChequeDeptDate,cdi.DeptImagePath,cdi.Remarks,
                bd.BankName as ChequesIssedBankName
                    FROM PayHeader pyh
                    INNER JOIN PaymentMethod pm
                    ON pyh.PMID = pm.PMID
                    INNER JOIN ChequesDeptInfo cdi
                    ON pyh.PHID = cdi.PayHeaderID
                    INNER JOIN Shops sp
                    ON sp.id = pyh.ShopID
                    INNER JOIN OwnerBankInfo obi
        ...
        if ($query_run) 
        {
            ...
            $data = [
                'status'=> 200,
                'message'=> 'Cheque Payment data fetched successfully',
                'data'=> $cashPayData,
            ];
            header('HTTP/1.0 200 Success');
            return json_encode($data);
