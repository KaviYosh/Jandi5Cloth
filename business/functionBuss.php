...
function getGarmentSectTotalCost(){

    /// Created By : Kavinda
    /// Date : 2026-05-14
    /// Description : get the Total Garment Section cost details

    global $conn;

    try {
       
        $queryCredit = "SELECT Sum(GartTotalPrice) AS TotGarmentSectCost FROM GarmentInvoiceHeader WHERE Active = 1";
        ...
        if ($queryCreditRun) {
            if (mysqli_num_rows($queryCreditRun) > 0) {
                ...
                $data = [
                    'status'=> 200,
                    'message'=> 'Total Garment Section Cost Fetched Successfully',
                    'data' => $res
                ];
                header('HTTP/1.0 200 OK');
                return json_encode($data);
                ...
                $data = [
                    'status'=> 404,
                    'message'=> 'No Garment Section Cost Found',
                ];
                header('HTTP/1.0 404 Not Found');
                return json_encode($data);
            }
...
function getGarmentSectTotalPayment(){
    ...
    /// Created By : Kavinda
    /// Date : 2026-05-14
    /// Description : get the Total Garment Section payment details
    ...
    try {
        ...
        if ($queryCreditRun) {
            if (mysqli_num_rows($queryCreditRun) > 0) {
                ...
                $data = [
                    'status'=> 200,
                    'message'=> 'Total Garment Section Payment Fetched Successfully',
                    'data' => $res
                ];
                header('HTTP/1.0 200 OK');
                return json_encode($data);
                ...
                $data = [
                    'status'=> 404,
                    'message'=> 'No Garment Section Payment Found',
                ];
                header('HTTP/1.0 404 Not Found');
                return json_encode($data);
            }
